...
const MultiAnglePlayerModal: React.FC<MultiAnglePlayerModalProps> = ({ videos, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-xl p-4 animate-in fade-in duration-300">
      <div className="relative w-full max-w-[95vw] h-full max-h-[90vh] rounded-3xl overflow-hidden shadow-2xl border border-slate-800 flex flex-col xl:flex-row bg-slate-950">
        
        <button onClick={onClose} className="absolute top-4 right-4 z-50 p-2 bg-black/50 hover:bg-black rounded-full text-white transition-colors">
          <X className="h-6 w-6" />
        </button>
        ...
        <div className="w-full xl:w-96 flex flex-col p-6 bg-slate-900/50 overflow-y-auto space-y-4">
...
export default MultiAnglePlayerModal;
